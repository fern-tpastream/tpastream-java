...
@JsonDeserialize(
    using = DeleteRsaException.Deserializer.class
)
public final class DeleteRsaException extends Exception {
  private final Value value;

  private int statusCode;

  private DeleteRsaException(Value value, int statusCode) {
    this.value = value;
  ...
  public static DeleteRsaException other(Object unknownValue, int statusCode) {
    return new DeleteRsaException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "DeleteRsaException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<DeleteRsaException> {
    @Override
    public DeleteRsaException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new DeleteRsaException(value, statusCode);
    }
  }
}
