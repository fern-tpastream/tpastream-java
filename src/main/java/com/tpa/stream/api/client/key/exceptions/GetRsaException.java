...
@JsonDeserialize(
    using = GetRsaException.Deserializer.class
)
public final class GetRsaException extends Exception {
  private final Value value;

  private int statusCode;

  private GetRsaException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetRsaException other(Object unknownValue, int statusCode) {
    return new GetRsaException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetRsaException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetRsaException> {
    @Override
    public GetRsaException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetRsaException(value, statusCode);
    }
  }
}
