...
@JsonDeserialize(
    using = CreateRsaException.Deserializer.class
)
public final class CreateRsaException extends Exception {
  private final Value value;

  private int statusCode;

  private CreateRsaException(Value value, int statusCode) {
    this.value = value;
  ...
  public static CreateRsaException other(Object unknownValue, int statusCode) {
    return new CreateRsaException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "CreateRsaException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<CreateRsaException> {
    @Override
    public CreateRsaException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new CreateRsaException(value, statusCode);
    }
  }
}
