...
@JsonDeserialize(
    using = GetGpgException.Deserializer.class
)
public final class GetGpgException extends Exception {
  private final Value value;

  private int statusCode;

  private GetGpgException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetGpgException other(Object unknownValue, int statusCode) {
    return new GetGpgException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetGpgException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetGpgException> {
    @Override
    public GetGpgException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetGpgException(value, statusCode);
    }
  }
}
