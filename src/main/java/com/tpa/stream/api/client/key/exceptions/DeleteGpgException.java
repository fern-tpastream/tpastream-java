...
@JsonDeserialize(
    using = DeleteGpgException.Deserializer.class
)
public final class DeleteGpgException extends Exception {
  private final Value value;

  private int statusCode;

  private DeleteGpgException(Value value, int statusCode) {
    this.value = value;
  ...
  public static DeleteGpgException other(Object unknownValue, int statusCode) {
    return new DeleteGpgException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "DeleteGpgException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<DeleteGpgException> {
    @Override
    public DeleteGpgException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new DeleteGpgException(value, statusCode);
    }
  }
}
