package com.tpa.stream.api.client.member.exceptions;
...
@JsonDeserialize(
    using = GetMemberException.Deserializer.class
)
public final class GetMemberException extends Exception {
  private final Value value;

  private int statusCode;

  private GetMemberException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetMemberException other(Object unknownValue, int statusCode) {
    return new GetMemberException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetMemberException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetMemberException> {
    @Override
    public GetMemberException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetMemberException(value, statusCode);
    }
  }
}
